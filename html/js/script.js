...
$(document).on('click', '.button', function(e){
    ...
    Radio.OnClick()
    if (KeyData === 'onoff') {
        $.post('https://qb-radio/ToggleOnOff', JSON.stringify({}));
    }
    if (Radio.Enabled) {
        if (KeyData === 'submit') {
            $.post('https://qb-radio/JoinRadio', JSON.stringify({
            ...
            $.post('https://qb-radio/SetVolume', JSON.stringify({
                Type: 'Up'
            }));
...
Radio.SlideDown = function() {
    $(".radio-container").animate({bottom: "-150vh",}, 400, function(){
